-- Characterisation harness schema for Sprint 7.
--
-- This is a copy of sprint-06-api/src/test/resources/schema.sql with the single
-- compatibility change required by the real PostgreSQL database the characterisation
-- tests run against: every `BIGINT AUTO_INCREMENT` becomes `BIGINT GENERATED BY
-- DEFAULT AS IDENTITY` (PostgreSQL's native identity syntax). Table and column shape
-- are otherwise identical, so the Sprint 6 sources see the same world they see under
-- the original test schema.
--
-- This file shadows the Sprint 6 test schema on the classpath (it lives in this
-- module's own default test resources). Keep it in sync if Sprint 6's schema evolves.
...
-- Only the terminal row carries a key, and it is what refuses a duplicate submission
-- now that the orders row does not outlive settlement.
CREATE UNIQUE INDEX uq_order_history_idempotency_key
    ON order_history (idempotency_key) WHERE idempotency_key IS NOT NULL;
